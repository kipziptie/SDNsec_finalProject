...
class SdnControllerClient():
    ...
    SELF_VERIFIER_THRESHOLD=100000
    ...
    def _block_tcp_traffic(self, port_id, protocol=None):

        if (protocol):
            data='{"in_port": "' + str(port_id) + '", "nw_proto": "TCP", "dl_type": "IPv4", "actions": "DENY", "priority": 100}'
        else:
            data='{"in_port": "' + str(port_id) + '", "dl_type": "IPv4", "actions": "DENY", "priority": 100}'
    ...
    def _enforceRulesBasedOnAttack(self, attack_type, port_id):
        if ( "ICMP" in attack_type ):
            ...
            self._block_tcp_traffic(port_id, "TCP")
        else:
